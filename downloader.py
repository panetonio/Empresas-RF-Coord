...
# ---------------------------------------------------------------------------
# Configuração da plataforma Nextcloud
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Conversão de encoding latin1 → utf8 via iconv
# ---------------------------------------------------------------------------
...
def _process_one_zip(
    ...
    Pipeline completo para um arquivo ZIP:
      ...
      3. Deleção do ZIP
      4. iconv: latin1 → utf8
      5. scan_csv utf8 → filtra ativos (ESTABELE) → Parquet brotli
      6. Deleção do CSV utf8

    Um arquivo Parquet por CSV de entrada.
    ...
    for csv_name in csv_names:
        ...
        # 5. Leitura, filtro de ativos (ESTABELE) e escrita em Parquet brotli
        if is_estabele:
            df = (
                pl.scan_csv(
                    utf8_path,
        ...
        # Compressão brotli para otimizar espaço
...
def download_all(
    ...
    Todos os Parquets são salvos em dest_dir (estrutura flat, sem subpastas).
